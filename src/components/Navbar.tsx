    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-out ${
        isScrolled 
          ...
          : 'bg-white/90 backdrop-blur-md border-b border-slate-100/30'
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            {/* Logo Section */}
            <button
              className="group flex items-center space-x-3 focus:outline-none focus:ring-2 focus:ring-golden/20 rounded-xl p-2 -m-2 transition-all duration-300"
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="relative group">
                <button
                  ...
                  className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-golden/10 to-amber-500/10 hover:from-golden/20 hover:to-amber-500/20 border border-golden/20 rounded-xl transition-all duration-300 group-hover:scale-105 group-hover:shadow-md group-hover:shadow-golden/10"
                >
                  <Globe className="w-4 h-4 text-golden transition-transform duration-300 group-hover:rotate-12" />
                  <span className="text-sm font-semibold text-golden uppercase tracking-wide">
                    {language}
                  </span>
                  <ChevronDown className="w-3 h-3 text-golden/70 transition-transform duration-300 group-hover:rotate-180" />
                </button>
              </div>
              ...
              <div className="md:hidden">
                <button
                  ...
                  className="relative p-2 text-slate-700 hover:text-golden transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-golden/20 rounded-xl group"
                  aria-label="Toggle menu"
                >
                  <div className="w-6 h-6 relative">
                    <span className={`absolute left-0 top-1.5 w-6 h-0.5 bg-current transform transition-all duration-300 origin-center ${
                      isMenuOpen ? 'rotate-45 translate-y-1.5' : ''
                    }`}></span>
                    <span className={`absolute left-0 top-3 w-6 h-0.5 bg-current transition-all duration-300 ${
                      isMenuOpen ? 'opacity-0 scale-0' : 'opacity-100'
                    }`}></span>
                    <span className={`absolute left-0 top-4.5 w-6 h-0.5 bg-current transform transition-all duration-300 origin-center ${
                      isMenuOpen ? '-rotate-45 -translate-y-1.5' : ''
                    }`}></span>
                  </div>
                </button>
            ...
            <div className="max-w-7xl mx-auto px-4 py-4 space-y-2">
              {navItems.map((item, index) => (
                <button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className={`group w-full text-left px-4 py-3 rounded-xl font-medium transition-all duration-300 transform ${
                    activeSection === item.id
                      ? 'text-golden bg-gradient-to-r from-golden/10 to-amber-500/10 border border-golden/20 shadow-sm'
                      : 'text-slate-700 hover:text-golden hover:bg-slate-50 hover:pl-6'
                  }`}
                  style={{ 
